...
def sample_rois_v4(rois, num_classes, cfg,
                   labels=None, overlaps=None, bbox_targets=None, gt_boxes=None):
    """
    generate random sample of ROIs comprising foreground and background examples
    :param rois: all_rois [n, 4]; e2e: [n, 5] with batch_index
    ...
    bg_ind = np.where(overlaps < cfg.TRAIN.BG_THRESH_HI)[0]
    labels[bg_ind] = 0
    nongt_inds = np.zeros((len(overlaps), 1), dtype=np.float32)
    nongt_inds[np.where(overlaps < 0.9999)[0]] = 1
    # load or compute bbox_target
    if bbox_targets is not None:
        bbox_target_data = bbox_targets
    else:
    ...
    return rois, labels, bbox_targets, bbox_weights, nongt_inds
...
def sample_rois_v3(rois, fg_rois_per_image, rois_per_image, num_classes, cfg,
                   labels=None, overlaps=None, bbox_targets=None, gt_boxes=None):
    """
    generate random sample of ROIs comprising foreground and background examples
    :param rois: all_rois [n, 4]; e2e: [n, 5] with batch_index
    ...
    labels[bg_ind] = 0
    rois = rois[keep_indexes]
    # gt_inds = np.where(overlaps[keep_indexes] > 0.9999)[0]
    nongt_inds = np.zeros((len(keep_indexes), 1), dtype=np.float32)
    nongt_inds[np.where(overlaps[keep_indexes] < 0.9999)[0]] = 1
