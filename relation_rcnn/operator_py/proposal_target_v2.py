...
class ProposalTargetV2Operator(mx.operator.CustomOp):
    def __init__(self, num_classes, batch_images, batch_rois, cfg, fg_fraction):
        super(ProposalTargetV2Operator, self).__init__()
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if self._batch_rois >= -10:
            all_rois = np.vstack((all_rois, np.hstack((zeros, gt_boxes[:, :-1]))))
        # Sanity check: single batch only
        assert np.all(all_rois[:, 0] == 0), 'Only single item batches are supported'

        if self._batch_rois == -1:
            rois, labels, bbox_targets, bbox_weights, nongt_inds = \
                sample_rois_v4(all_rois, self._num_classes, self._cfg, gt_boxes=gt_boxes)
        else:
            rois, labels, bbox_targets, bbox_weights, nongt_inds = \
                sample_rois_v3(all_rois, fg_rois_per_image, rois_per_image, self._num_classes, self._cfg, gt_boxes=gt_boxes)
        ...
        for ind, val in enumerate([rois, labels, bbox_targets, bbox_weights, nongt_inds]):
...
@mx.operator.register('proposal_target_v2')
class ProposalTargetV2Prop(mx.operator.CustomOpProp):
    def __init__(self, num_classes, batch_images, batch_rois, cfg, fg_fraction='0.25'):
        super(ProposalTargetV2Prop, self).__init__(need_top_grad=False)
    ...
    def infer_shape(self, in_shape):
        ...
        nongt_inds_shape = (rois, 1) 
        return [rpn_rois_shape, gt_boxes_shape], \
               [output_rois_shape, label_shape, bbox_target_shape, bbox_weight_shape, nongt_inds_shape]

    def create_operator(self, ctx, shapes, dtypes):
        return ProposalTargetV2Operator(self._num_classes, self._batch_images, self._batch_rois, self._cfg, self._fg_fraction)
